...
export const Sidebar: FC<IProps> = ({ isOpen }) => {
  ...
  return (
    ...
    >
      <div className="mx-3 my-16 md:flex hidden flex-col gap-y-3 items-center fixed">
        <SideButtons link="/" routename="Главная страница">
          <HiSquaresPlus className="text-white text-3xl" />
        </SideButtons>
        <SideButtons link="/users" routename="Пользователи">
          <FaUser className="text-white text-2xl" />
        </SideButtons>
        <SideButtons link="/chats" routename="Чаты">
          <HiChatBubbleBottomCenter className="text-white text-3xl" />
        </SideButtons>
        <SideButtons link="/categories" routename="Категории">
          <MdCategory className="text-white text-3xl" />
        </SideButtons>
        <SideButtons link="/complaints" routename="Жалобы">
